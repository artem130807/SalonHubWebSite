  ...
  return (
    <SiteShell>
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
        ...
        <form className="flex flex-col md:flex-row items-center gap-0 bg-surface/50 p-2 rounded-3xl border border-outline/50 shadow-sm relative z-10 w-full">
          <label className="flex items-center gap-4 flex-1 w-full px-6 py-4 group">
            ...
            <input
              ...
              className="w-full bg-transparent outline-none text-base sm:text-lg py-1 placeholder:text-onSurfaceVariant/70"
            />
          </label>
        ...
        <div className="flex gap-2.5 overflow-x-auto hide-scrollbar pb-1 -mx-4 px-4 sm:mx-0 sm:px-0 sm:flex-wrap">
          {SERVICE_CATEGORIES.map((item) => (
            <Link
              key={item.id}
              href={salonsHref({ q, sort, category: category === item.query ? undefined : item.query })}
              className={`shrink-0 px-4 py-2 rounded-full text-sm font-medium transition-all ${category === item.query ? "bg-primary text-onPrimary shadow-md" : "bg-surface border border-outline text-onSurface hover:border-primary hover:text-primary"}`}
            >
              {item.label}
            </Link>
          ))}
          ...
          {sorts.map((item) => (
            <Link
              ...
              className={`shrink-0 px-4 py-2 rounded-full text-sm font-medium transition-all ${sort === item.id ? "bg-primary text-onPrimary shadow-md" : "bg-surface border border-outline text-onSurface hover:border-primary hover:text-primary"}`}
            >
              {item.label}
            </Link>
          ))}
